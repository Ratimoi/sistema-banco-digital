...
- Projeto renomeado de "Sistema Bancário Digital" para **TeenBank** (logo, título das páginas,
  ...
  título do workflow de CI). O nome do repositório no GitHub não mudou.
- URLs de produção migradas para o domínio novo: frontend em `https://teenbank.onrender.com` e
  backend em `https://teenbank-api.onrender.com` (o Render não permite trocar o subdomínio de um
  serviço existente, então foram criados serviços novos espelhando a configuração dos antigos,
  incluindo as variáveis de ambiente). Os serviços antigos (`sistema-banco-digital*.onrender.com`)
  foram desligados após a migração ser validada em produção.
